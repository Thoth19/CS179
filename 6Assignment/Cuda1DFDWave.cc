...
int main(int argc, char* argv[]) {
...

	{
		...
		cudaMalloc((void **) &dev_data[2], numberOfNodes * sizeof(float));
                ...
                cudaMemset(dev_data[0], 0, numberOfNodes * sizeof(float));
                cudaMemset(dev_data[1], 0, numberOfNodes * sizeof(float));
                cudaMemset(dev_data[2], 0, numberOfNodes * sizeof(float));
		...
		for (size_t timestepIndex = 0; timestepIndex < numberOfTimesteps;
			...
			cudaMemcpy(dev_newDisplacements + (numberOfNodes-1), &zero, sizeof(float), cudaMemcpyHostToDevice);
                        //cudaMemcpy(dev_newDisplacements + 5, &asdf, sizeof(float), cudaMemcpyHostToDevice);
			...
			if (CUDATEST_WRITE_ENABLED == true && numberOfOutputFiles > 0 &&
				...
				cudaMemcpy(file_output, dev_newDisplacements, numberOfNodes*sizeof(float), cudaMemcpyDeviceToHost);
